...
'''
Plugin para modificar el contraste de las imágenes
'''
        ...
        for i in range(len(self.imagenesIniciales)):
            ...
            if im:
                if self.ventana.automatico.isChecked():
                   ...
                   im=enh.enhance(1.3) #Contraste al 30%
                else:
                   contraste=self.ventana.horizontalSlider.value()
                   enh = ImageEnhance.Contrast(im)
                   im=enh.enhance((float(contraste)+100)/100)
...
class Contraste(PluginBase):
    ...
    def descripcion(self):
        return 'Altera el contraste de las imágenes'
    ...
    def icon(self):
        return os.path.dirname(__file__) + '/images/contraste.png'
